...
func main() {
	places := handleCommandLine(1000) //小数位数
	scaledPi := fmt.Sprint(π(places)) //把结果打印到一个字符串
	fmt.Printf("3.%s\n", scaledPi[1:])
}

func handleCommandLine(defaultValue int) int { //参数输入
	if len(os.Args) > 1 { //如果有参数
		if os.Args[1] == "-h" || os.Args[1] == "--help" { //如果参数等于 -h 或者 --help
			usage := "usage: %s [digits]\n e.g.: %s 10000" //使用示例
			app := filepath.Base(os.Args[0])               //获取程序名
			fmt.Fprintln(os.Stderr, fmt.Sprintf(usage, app, app))
			os.Exit(1)
		}
		if x, err := strconv.Atoi(os.Args[1]); err != nil { //如果参数错误
